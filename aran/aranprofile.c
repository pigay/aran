  ...
  for (i=0; i<nsamples; i ++)
    {
      ...
      ARAN_PROFILE_CODE_NUMBASE ({/* init (p); */}, {accum = m2pi (&dstinfo, dst, &p);},
                                 t, _TIMEBASE2, _NUMBASE2);
...
static void vsg_vector3d_set_zero (VsgVector3d *v)
{
...
gdouble aran_poly1d_profile_m2pvi_3d_samples (AranMultipole2ParticleGradInternalFunc3d m2pvi,
                                              AranParticleInitFunc3d init,
                                              AranDevelopmentNewFunc _new,
                                              GDestroyNotify _free,
                                              AranPoly1d *ap1d,
  ...
  for (i=0; i<nsamples; i ++)
    {
      ...
      ARAN_PROFILE_CODE_NUMBASE ({vsg_vector3d_set_zero (&g);}, {m2pvi (&dstinfo, dst, &p, &g);},
                                 t, _TIMEBASE2, _NUMBASE2);
